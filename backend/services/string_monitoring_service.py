...
class StringMonitoringService:
    ...
    def process_strings(self, inverter_id: int, strings_data: List[Dict], polling_time: str) -> List[Dict]:
        ...
        if inverter_id not in self.baseline_date:
            self.baseline_date[inverter_id] = None

        # =========================================================
        # 1. CHỐT BASELINE DUY NHẤT 1 LẦN TRONG NGÀY (lúc >= 7h)
        # =========================================================
        if current_hour >= 7 and self.baseline_date[inverter_id] != today:
            baseline = {}
            ...
            for s in strings_data:
                ...
                # Ngưỡng tránh nhiễu
                baseline[idx] = 1 if current_i > 0.2 else 0

            self.morning_baseline[inverter_id] = baseline
            self.baseline_date[inverter_id] = today

            logger.info(f"[StringMonitor] Baseline captured for inverter {inverter_id}: {baseline}")
            return []

        # =========================================================
        # 2. CHỈ TRACK TỪ 7h → 16h
        # =========================================================
